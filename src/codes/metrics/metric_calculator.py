...
class MetricCalculator():
    ...
    def compute_sequence_metrics(self, seq, true_seq_dir, pred_seq_dir,
        ...
        # retrieve files if true_seq is not provided
        if true_seq is None:
        ...
        for i in range(len(true_seq) if true_seq is not None else len(true_img_lst)):
            # use given true_seq or load from disk
            if true_seq is not None:
                self.true_img_cur = true_seq[i]  # hwc|rgb|uint8
            else:
                self.true_img_cur = cv2.imread(true_img_lst[i], cv2.IMREAD_UNCHANGED)[..., ::-1]  # bgr2rgb
    ...
    def compute_LPIPS(self):
        ...
        mean = [0.5, 0.5, 0.5]
        std = [0.5, 0.5, 0.5]
        normalize(true_img, mean, std, inplace=True)
        normalize(pred_img, mean, std, inplace=True)

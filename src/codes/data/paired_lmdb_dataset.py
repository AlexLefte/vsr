...
class PairedLMDBDatasetV2(BaseDataset):
    def __init__(self, data_opt, **kwargs):
        """ LMDB dataset with paired data, for BI degradation
        """
        super(PairedLMDBDatasetV2, self).__init__(data_opt, **kwargs)
    ...
    def __getitem__(self, item):
        ...
        # Get info
        seq_idx, idx, (tot_frm, gt_h, gt_w), _ = self.parse_lmdb_key(gt_key)       
        _, _, (_, lr_h, lr_w), _ = self.parse_lmdb_key(lr_key)
        c, s = 3, self.scale
        ...
        # Pregătește cadrele pentru antrenare
        gt_frms, lr_frms = [], []
        if self.train and self.moving_first_frame and (random.uniform(0, 1) > self.moving_factor):
            ...
            gt_frm = self.read_lmdb_frame(
                self.gt_env, gt_key, size=(gt_h, gt_w, c), dtype=self.gt_dtype)
            gt_frm = gt_frm.transpose(2, 0, 1)  # chw|rgb|uint16
            ...
            for i in range(cur_frm, cur_frm + self.tempo_extent):
                if i >= tot_frm:
                    ...
                    gt_key = '{}_{}_{}x{}x{}_{:04d}'.format(
                        seq_idx, idx, tot_frm, gt_h, gt_w, 2 * tot_frm - i - 2)
                    lr_key = '{}_{}_{}x{}x{}_{:04d}'.format(
                        seq_idx, idx, tot_frm, lr_h, lr_w, 2 * tot_frm - i - 2)
                else:
                    gt_key = '{}_{}_{}x{}x{}_{:04d}'.format(
                        seq_idx, idx, tot_frm, gt_h, gt_w, i)
                    lr_key = '{}_{}_{}x{}x{}_{:04d}'.format(
                        seq_idx, idx, tot_frm, lr_h, lr_w, i)
        ...
        # Normalizează
        gt_tsr = torch.FloatTensor(np.ascontiguousarray(gt_frms)) / (2 ** self.gt_bit_depth - 1)
    ...
    def parse_lmdb_key(key):
        key_lst = key.split('_')
        idx, clip_idx, size, frm = key_lst[0], key_lst[1:-2], key_lst[-2], int(key_lst[-1])
        clip_idx = '_'.join(clip_idx)
        size = tuple(map(int, size.split('x')))  # n_frm, h, w
        return idx, clip_idx, size, frm

    @staticmethod
    def read_lmdb_frame(env, key, size, dtype=np.uint8):
        with env.begin(write=False) as txn:
            buf = txn.get(key.encode('ascii'))
        frm = np.frombuffer(buf, dtype=dtype).reshape(*size)
        return frm
